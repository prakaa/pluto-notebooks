...
In this [Pluto.jl notebook](https://www.youtube.com/watch?v=IAF8DjrQSSk), I have  adapted a simple economic dispatch model to demonstrate how reserves (e.g. Frequency Control Ancillary Services or FCAS in Australia's National Electricity Market) can be co-optimised with energy in a centrally dispatched electricity market. 

Specifically, this notebook explains how this co-optimisation enables the *opportunity-cost* of reserve provision to be incorporated into pricing.

The example and code borrows heavily from the economic dispatch tutorial available in [JuMP's documentation](https://jump.dev/JuMP.jl/stable/tutorials/applications/power_systems/). JuMP is an optimisation framework written for Julia.
"""
...
If you're not familiar with the term, *economic dispatch* (ED) is an optimisation problem:

- If the system is run by a central utility, this process can help the utility decide how to meet demand at lowest cost.
- If the power system has an energy market (from here on, energy is just another word for electricity), generators can *offer* their energy at various prices. In some systems, a market operator runs ED to decide which offers from generators (and in some cases, bids from loads) in the market should be accepted to run the system at lowest cost. The process also produces *marginal prices* for energy and reserves, which sets the price that all generators supplying these products receive. This is how we run the energy market in Australia (called the National Electricity Market or NEM).
...
ED is an economic/market problem. For example, the limits or *constraints* we include below do not consider technical requirements (e.g. appropriate power flows to maintain stability) and limitations (e.g. power flow below transmission line ratings) of the power system. However, if such constraints are added, we can call the problem *security-constrained economic dispatch*.

### Complexity

Economic dispatch can result in price and dispatch outcomes that are not obvious and perhaps even counter-intuitive. Transpower NZ has done a [great video series](https://youtube.com/playlist?list=PLXUccGn4ptEO5e0-MV37_vWPWerhB8yak) on marginal pricing and the "spring-washer" effect that can arise due to network constraints. Another factor that can complicate economic dispatch is when reserves are included in economic dispatch and *co-optimised* with energy. We will explore this below.

## What are reserves?

The examples below run economic dispatch for a single time interval. In reality, ED is run at a regular frequency (every 5 minutes in the NEM). 

Between one ED run and another: 
...
- Generators/loads tripping unexpectedly
- Renewable energy forecast errors

To provide reserves that can help increase supply (*raise* reserves) or decrease supply (*lower* services), generators may need to be "turned up or down" in the energy market. 

We explore how the optimisation problem and its solution accounts for this and ensures that prices incorporate any opportunity-cost (skip to the last example if you're interested in this).
"""
...
- short-run marginal cost (\$/MW/hr) is given by ``c_i^g``
...
Shadow price of energy balance constraint (supply = demand): $(shadow_price(con)), so marginal cost is \$$(shadow_price(con)*-1)/MW/hr. This is the price of energy.
...
- short-run marginal cost of energy (\$/MW/hr) is given by ``c_i^g``
- ``r_i`` is the reserve provided by a dispatchable generator (Generator 1, 2 or 3)

- short-run marginal cost of reserve (\$/MW/hr) is given by ``r_i^g``
...
Objective value: \$ $obj_r

Shadow price of energy balance constraint (supply = demand): $(shadow_price(en_con)), so marginal cost is \$$(shadow_price(en_con)*-1)/MW/hr. This is the price of energy.

Shadow price of reserve requirement constraint (∑r = R): $(shadow_price(r_con)), so marginal cost is \$$(shadow_price(r_con)*-1)/MW/hr. This is the price of reserves.
...
The sum of these is 280, the price of reserves. 

Simply put, **if a unit is backed off energy to provide reserves (e.g. FCAS), the optimisation will ensure that it does not miss out on any profits so long as its bids reflect short run marginal costs**
"""
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "b882eec59c9b00f47464981fa0739ff9dd1ee073"
